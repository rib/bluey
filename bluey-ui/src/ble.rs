...
impl BleService {
    ...
    pub async fn bluetooth_service(event_proxy: EventLoopProxy<crate::ui::Event>,
        ...
        let session = bmb::session::SessionConfig::new().start().await?;
        ...
        while let Some((_, event)) = mainloop.next().await {
            match event {
                Event::UiRequest(req) => {
                    match req {
                        ...
                        BleRequest::ReadGattCharacteristic(characteristic) => {
                            match characteristic.read_value(bmb::CacheMode::Uncached).await {
                                Ok(value) => {
                        ...
                        BleRequest::ReadGattDescriptor(descriptor) => {
                            match descriptor.read_value(bmb::CacheMode::Uncached).await {
                                Ok(value) => {
                ...
                Event::BtEvent(event) => {
                    match &event {
                        bmb::Event::PeripheralFound { peripheral, address, name, .. } => {
                        ...
                        /*
                        bmb::Event::PeripheralPrimaryGattServicesComplete { peripheral, .. } => {
                        ...
                        bmb::Event::ServiceIncludedGattServicesComplete { peripheral, service, .. } => {
                        ...
                        bmb::Event::ServiceGattCharacteristicsComplete { peripheral, service, .. } => {
