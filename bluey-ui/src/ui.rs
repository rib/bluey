...
use bmb::{peripheral::Peripheral, uuid::BluetoothUuid, service::Service, characteristic::{Characteristic, CharacteristicProperties}, descriptor::Descriptor};
...
const HEART_RATE_MEASUREMENT_CHARACTERISTIC_UUID: Uuid = bmb::uuid::uuid_from_u16(0x2A37);
...
impl State {
    ...
    fn update_selected_peripheral_property(&mut self, peripheral: &Peripheral, property_id: bmb::PeripheralPropertyId) {
        match property_id {
            bmb::PeripheralPropertyId::Name => {
            ...
            bmb::PeripheralPropertyId::AddressType => {
            ...
            bmb::PeripheralPropertyId::Rssi => { /* queried each frame */},
            bmb::PeripheralPropertyId::TxPower => { /* queried each frame */},
            bmb::PeripheralPropertyId::ManufacturerData => {
                self.selected_peripheral_state.as_mut().unwrap().manufacturer_data = peripheral.all_manufacturer_data();
            },
            bmb::PeripheralPropertyId::ServiceData => {

            },
            bmb::PeripheralPropertyId::ServiceIds => {
                self.selected_peripheral_state.as_mut().unwrap().service_ids = peripheral.service_ids();
            },
            bmb::PeripheralPropertyId::PrimaryServices => {},
            _ => {},
        }
    }
    ...
    fn select_peripheral(&mut self, peripheral: &Peripheral, connected: bool, ble_tx: &UnboundedSender<ble::BleRequest>) {
        ...
        self.update_selected_peripheral_property(peripheral, bmb::PeripheralPropertyId::Name);
        self.update_selected_peripheral_property(peripheral, bmb::PeripheralPropertyId::AddressType);
    ...
    pub fn handle_event(&mut self, event: Event, ble_tx: &UnboundedSender<ble::BleRequest>) {
        match event {
            Event::Ble(bmb::Event::PeripheralFound { peripheral, name, address, ..}) => {
                if !self.peripherals.contains(&peripheral) {
                    self.peripherals.insert(peripheral);
                    info!("UI: peripheral found {name}: {address}");
                }
            },
            Event::Ble(bmb::Event::PeripheralPropertyChanged { ref peripheral, property_id, ..}) => {
            ...
            Event::Ble(bmb::Event::PeripheralConnected { peripheral, ..}) => {
                self.select_peripheral(&peripheral, true, ble_tx);
                self.state = BleState::Connected;
                let _ = ble_tx.send(BleRequest::DiscoverGattServices(peripheral.clone()));
            }
            Event::Ble(bmb::Event::PeripheralDisconnected { peripheral, ..}) => {
            ...
            Event::Ble(bmb::Event::PeripheralPrimaryGattServicesComplete { peripheral, .. })=> {
                match &self.selected_peripheral {
                    Some(selected) if selected == &peripheral => {
            ...
            Event::Ble(bmb::Event::ServiceIncludedGattServicesComplete { peripheral, service, .. }) => {
            ...
            Event::Ble(bmb::Event::ServiceGattCharacteristicsComplete { peripheral, service, .. }) => {
                match &self.selected_service {
                    Some(selected) if selected == &service => {
            ...
            Event::Ble(bmb::Event::ServiceGattDescriptorsComplete { peripheral, service, characteristic, .. }) => {
            ...
            Event::Ble(bmb::Event::ServiceGattCharacteristicValueNotify { characteristic, value, .. }) => {
