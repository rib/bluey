...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let ctrl_c_stream: Pin<Box<dyn Stream<Item=Event>>> = Box::pin(signal::ctrl_c().into_stream().map(|_| { println!("Ctrl-C"); Event::Interrupt }));
    mainloop.insert(EventSource::Interrupt, ctrl_c_stream);

    let bt_event_stream: Pin<Box<dyn Stream<Item=Event>>> = Box::pin(events.map(|bt_event| Event::BtEvent(bt_event)));
    ...
    while let Some((_, event)) = mainloop.next().await {
        match event {
            Event::BtEvent(event) => {
                match event {
                    bmb::Event::PeripheralFound { peripheral: _, name, address, .. } => {
                        println!("Peripheral Found: {}: {}", name, address);
                    }
                    bmb::Event::PeripheralPropertyChanged(peripheral, property_id) => {
                        println!("Peripheral Property Changed: {}: {:?}", peripheral.address(), property_id);
                    }
                    _ => {
